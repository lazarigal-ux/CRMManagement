...
public sealed class LeadService : ILeadService
{
    ...
    public async Task<IReadOnlyList<LeadListItemDto>> ListAsync(CancellationToken ct)
    {
        return await _db.Leads.AsNoTracking()
            ...
            .Select(x => new LeadListItemDto(x.Id, x.FirstName, x.LastName, x.Company, x.Email, x.Status, x.Rating, x.Score, x.CreatedAt)
            {
                ...
                State            = x.State,
                Country          = x.Country,
                ZohoCreatedTime  = x.ZohoCreatedTime,
                ZohoModifiedTime = x.ZohoModifiedTime,
                OwnerName        = x.OwnerUserId == null
                    ? null
                    : _db.Users.Where(u => u.Id == x.OwnerUserId).Select(u => u.UserName).FirstOrDefault(),
            })
            .ToListAsync(ct);
    }
    ...
    public async Task<LeadDetailDto?> GetAsync(Guid id, CancellationToken ct)
    {
        ...
        return new LeadDetailDto(x.Id, x.FirstName, x.LastName, x.Company, x.Title, x.Email, x.Phone, x.Mobile, x.Source, x.Status, x.Rating, x.Score, x.OwnerUserId, x.Industry, x.Website, x.Description, x.AnnualRevenue, x.NoOfEmployees, x.Street, x.City, x.State, x.ZipCode, x.Country, x.ConvertedAccountId, x.ConvertedContactId, x.ConvertedOpportunityId, x.ConvertedAt);
    }
